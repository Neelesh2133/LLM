...
## 9. Full `GPTModel` Implementation & Parameter Breakdown

The complete `GPTModel` integrates token embeddings, positional embeddings, `n_layers` of `TransformerBlock` instances, final layer normalization, and the output linear head:
...
### Parameter Calculation (`GPT_CONFIG_124M`)

When instantiated with `GPT_CONFIG_124M`:

1. **Total Parameters (Raw Count)**:
   $$\text{Total Parameters} = \sum p.\text{numel}() = 163,009,536 \quad (\approx 163\text{M})$$
   - `tok_emb.weight`: `[50257, 768]` $\rightarrow$ $38,597,376$ parameters.
   - `out_head.weight`: `[50257, 768]` $\rightarrow$ $38,597,376$ parameters.

2. **Weight Tying Concept**:
   - In standard GPT-2 architectures, weight tying reuses `tok_emb` weights for `out_head`.
